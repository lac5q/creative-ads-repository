#!/usr/bin/env python3
"""
Airtable Table Creator for Video Data
...
def create_video_table(api_key, app_id, table_name="Veo3 Videos"):
    ...
    # Define the table structure based on your CSV
    table_structure = {
        "name": table_name,
        "fields": [
...
def upload_to_new_table(api_key, app_id, table_name, csv_file):
    ...
    for idx, row in df.iterrows():
        ...
        for key, value in row.items():
            ...
            if key in ['Spend'] and value != 'TBD':
                try:
                    record["fields"][key] = float(value)
                except:
                    continue
            elif key in ['Purchases', 'Video_Views', 'Ad_ID', 'Creative_ID'] and value != 'TBD':
                try:
                    record["fields"][key] = int(float(value))
                except:
...
def main():
    print("🚀 AIRTABLE VIDEO TABLE CREATOR & UPLOADER")
    print("="*60)
    ...
    if table_name:
        print(f"\n⏳ Waiting 2 seconds for table to be ready...")
        time.sleep(2)
